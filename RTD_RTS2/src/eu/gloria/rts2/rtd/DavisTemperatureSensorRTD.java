 ...
 *
 */
public class DavisTemperatureSensorRTD  extends DeviceRTD implements RTDTemperatureSensorInterface{
	...
	public double tempGetMeasure() throws RTException {
		...
		DeviceProperty property = this.devGetDeviceProperty("DOME_TMP");
		...
		if (dev.getActivityState() != ActivityState.ERROR){
			if (dev.getAlarmState() == AlarmState.NONE){
				if (allProperties){
					...
					devProperty = devGetDeviceProperty("DOME_TMP");
