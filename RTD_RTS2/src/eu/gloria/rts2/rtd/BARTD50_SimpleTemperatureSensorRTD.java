 ...
 *
 */
public class BARTD50_SimpleTemperatureSensorRTD  extends DeviceRTD implements RTDTemperatureSensorInterface{
	...
	public double tempGetMeasure() throws RTException {
		...
		DeviceProperty property = this.devGetDeviceProperty("TEMP_SIMPLE");
	...
	public Device devGetDevice(boolean allProperties)  throws RTException{
		...
		//Description		
		dev.setDescription(this.devGetDeviceProperty("TEMP_SIMPLE").getDescription());
		//Info
		...
		if (dev.getAlarmState() == AlarmState.NONE){
			if (allProperties){
				...
				devProperty = devGetDeviceProperty("TEMP_SIMPLE");
