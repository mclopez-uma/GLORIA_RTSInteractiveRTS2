 ...
 *
 */
public class BARTD50_DomeTemperatureSensorRTD  extends DeviceRTD implements RTDTemperatureSensorInterface{
	...
	public double tempGetMeasure() throws RTException {
		...
		DeviceProperty property = this.devGetDeviceProperty("TEMP_DOME");
	...
	public Device devGetDevice(boolean allProperties)  throws RTException{
		...
		//Description		
		dev.setDescription(this.devGetDeviceProperty("TEMP_DOME").getDescription());
		//Info
		...
		if (dev.getAlarmState() == AlarmState.NONE){
			if (allProperties){
				...
				devProperty = devGetDeviceProperty("TEMP_DOME");
